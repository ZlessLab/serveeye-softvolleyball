...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return res.status(503).json({ error: 'No active plan', detail: err.message });
  }
  ...
  if (plan.campaign) {
    try {
      ...
      const { data: planRow } = await supabase.from('plans').select('id').eq('slug', plan.slug).single();
      if (planRow) {
      ...
      console.error('Campaign check error:', err.message);
    }
  }
